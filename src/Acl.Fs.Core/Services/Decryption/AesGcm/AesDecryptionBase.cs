    ...
    IFileVersionValidator versionValidator) : IAesDecryptionBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var plaintext = BufferPool.Rent(BufferSize);
        var alignedBuffer = BufferPool.Rent(BufferSize);
        var metadataBuffer = BufferPool.Rent(SectorSize);
        var tag = BufferPool.Rent(TagSize);
        var chunkNonce = BufferPool.Rent(NonceSize);
        var salt = BufferPool.Rent(SaltSize);
        ...
        finally
        {
            ...
            BufferPool.Return(plaintext, true);
            BufferPool.Return(alignedBuffer, true);
            BufferPool.Return(metadataBuffer, true);
            BufferPool.Return(tag, true);
            BufferPool.Return(chunkNonce, true);
            BufferPool.Return(salt, true);
        }
    }
